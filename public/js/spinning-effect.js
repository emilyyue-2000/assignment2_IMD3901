...
AFRAME.registerComponent('spinning-effect', {
  ...
  init: function() {
    ...
    Context_AF.cubes      = document.querySelector('#dogCube');
    Context_AF.isSpinning = false;
    
    //let's add the basic animation to teh walls entity
    //note that it is not enabled initially
    Context_AF.cubes.setAttribute('animation', {property:'rotation.y', to:360, loop:true, dur:Context_AF.data.duration, easing:'linear', enabled:false});
    
    //listen on click
    Context_AF.el.addEventListener('click', function() {
      if (Context_AF.isSpinning === true) {
        console.log('stop spinning');
        Context_AF.cubes.setAttribute('animation', {enabled:false});
        Context_AF.isSpinning = false;
      }
      else {
        console.log('spinning');
        Context_AF.cubes.setAttribute('animation', {enabled:true});
